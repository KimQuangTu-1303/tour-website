export function initFlightListing() {
  ...
  function runFlightFilter() {
    ...
    const currentMaxPrice = priceMax ? parseInt(priceMax.value) : Infinity;
  ...
  document.querySelectorAll('input[name="rating"]').forEach((cb) => {
    cb.addEventListener("click", function () {
      ...
      if (currentBaseRating === clickedValue) {
        document.querySelectorAll('input[name="rating"]').forEach((otherCb) => (otherCb.checked = false));
        currentBaseRating = null;
      } else {
        document.querySelectorAll('input[name="rating"]').forEach((otherCb) => {
          otherCb.checked = parseInt(otherCb.value) >= clickedValue;
        });
  ...
  function renderTickets(ticketsData) {
    ...
    ticketsData.forEach((ticket) => {
      ...
      if (priceUnitEl) priceUnitEl.textContent = ticket.priceUnit || "";
  ...
  function initFavoritesUI() {
    ...
    favoriteCheckboxes.forEach((checkbox) => {
      ...
      // Xóa tác vụ clone không cần thiết, gắn sự kiện trực tiếp
